<?php
  ...
  /* $flag= mobileIdFinder($user_mobile,$statusType);
   ...
   if(!empty($user_mobile))
   {
	   ...
	   $quryStr="SELECT * FROM invoice WHERE (FAB_MOBILE ='$user_mobile' and FAB_INV_DATE <> '') order by INV_ID DESC"; 
	   else
	   $quryStr="SELECT * FROM invoice WHERE (RET_MOBILE ='$user_mobile' and RET_INV_DATE <> '' ) order by INV_ID DESC"; 
   ...
   if(!empty($user_mobile) && !empty($statusType)&& $statusType!='null')
   {
	   if($statusType=='All')
		...
		{
			if($user_type == 'Fabricator')
			$quryStr="SELECT * FROM invoice WHERE (FAB_MOBILE ='$user_mobile' and FAB_INV_DATE <> '') and INV_STATUS_TYPE='$statusType' order by INV_ID DESC"; 
			else
			$quryStr="SELECT * FROM invoice WHERE (RET_MOBILE ='$user_mobile' and RET_INV_DATE <> '' ) and INV_STATUS_TYPE='$statusType' order by INV_ID DESC"; 
	   }
   ...
   {
	 if($user_type == 'Fabricator'){
	 	$quryStr="SELECT * FROM invoice WHERE (FAB_MOBILE ='$user_mobile' and (CAST(FAB_INV_DATE As DATE) BETWEEN CAST('$startDate' As DATE) and CAST('$endDate' As DATE))) order by INV_ID DESC"; 
	 }else{
	 	$quryStr="SELECT * FROM invoice WHERE (RET_MOBILE ='$user_mobile' and (CAST(RET_INV_DATE As DATE) BETWEEN CAST('$startDate' As DATE) and CAST('$endDate' As DATE))) order by INV_ID DESC"; 
	 }
   }
   if(!empty($user_mobile) && !empty($startDate) && !empty($endDate) && $startDate!='null' && $endDate!='null' && !empty($statusType) && $statusType!='null')
   {
	    if($statusType=='All')
	    {
		    ...
		    //$quryStr="SELECT * FROM invoice WHERE (FAB_MOBILE ='$user_mobile' or RET_MOBILE ='$user_mobile') and (CAST(RET_INV_DATE As DATE) BETWEEN CAST('$startDate' As DATE) and CAST('$endDate' As DATE)) and INV_STATUS_TYPE = '$statusType'";  
	   		if($user_type == 'Fabricator'){
				$quryStr="SELECT * FROM invoice WHERE (FAB_MOBILE ='$user_mobile' and (CAST(FAB_INV_DATE As DATE) BETWEEN CAST('$startDate' As DATE) and CAST('$endDate' As DATE))) and INV_STATUS_TYPE = '$statusType' order by INV_ID DESC";  
			}else{
				$quryStr="SELECT * FROM invoice WHERE (RET_MOBILE ='$user_mobile' and (CAST(RET_INV_DATE As DATE) BETWEEN CAST('$startDate' As DATE) and CAST('$endDate' As DATE))) and INV_STATUS_TYPE = '$statusType' order by INV_ID DESC";  
			}
	   }
